...
function Invoke-Checked {
  ...
  Write-Host "[windows-production-signed] $Label"
  & $Command
...
try {
  ...
  Invoke-Checked "Build production payload directory" {
    & $node $builder --win dir --config.win.signAndEditExecutable=false --config.win.forceCodeSigning=false
  }

  Invoke-Checked "Production-sign unpacked SmartRead.exe" {
    & powershell -NoProfile -ExecutionPolicy Bypass -File $signScript -UnpackedOnly -Production
  }

  Invoke-Checked "Build NSIS installer from production-signed payload" {
  ...
  Invoke-Checked "Production-sign NSIS installer" {
    & powershell -NoProfile -ExecutionPolicy Bypass -File $signScript -InstallerOnly -Production
  }
} finally {
  Pop-Location
}
